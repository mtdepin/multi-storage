...
import (
	...
	"k8s.io/utils/clock"
	"k8s.io/utils/trace"
	"mtcloud.com/mtstorage/cmd/controller/app/informers/tools/pager"
...
func DefaultWatchErrorHandler(r *Reflector, err error) {
	...
	case isExpiredError(err):
		...
		logger.Infof("%s: watch of %v closed with: %v", r.name, r.expectedTypeName, err)
	case err == io.EOF:
		// watch closed normally
	case err == io.ErrUnexpectedEOF:
		logger.Infof("%s: Watch for %v closed with unexpected EOF: %v", r.name, r.expectedTypeName, err)
	default:
...
func (r *Reflector) setExpectedType(expectedType interface{}) {
	...
	if obj, ok := expectedType.(*unstructured.Unstructured); ok {
		...
		if gvk.Empty() {
			logger.Infof("Reflector from %s configured with expectedType of *unstructured.Unstructured with empty GroupVersionKind.", r.name)
			return
		}
		r.expectedGVK = &gvk
...
func (r *Reflector) Run(stopCh <-chan struct{}) {
	logger.Infof("Starting reflector %s (%s) from %s", r.expectedTypeName, r.resyncPeriod, r.name)
	wait.BackoffUntil(func() {
		if err := r.ListAndWatch(stopCh); err != nil {
			r.watchErrorHandler(r, err)
		}
	}, r.backoffManager, true, stopCh)
	logger.Infof("Stopping reflector %s (%s) from %s", r.expectedTypeName, r.resyncPeriod, r.name)
}
...
func (r *Reflector) ListAndWatch(stopCh <-chan struct{}) error {
	logger.Infof("Listing and watching %v from %s", r.expectedTypeName, r.name)
	...
	go func() {
		...
		for {
			...
			if r.ShouldResync == nil || r.ShouldResync() {
				logger.Infof("%s: forcing resync", r.name)
		...
		if err := r.watchHandler(start, w, &resourceVersion, resyncerrc, stopCh); err != nil {
			if err != errorStopRequested {
				...
				case apierrors.IsTooManyRequests(err):
					logger.Infof("%s: watch of %v returned 429 - backing off", r.name, r.expectedTypeName)
					<-r.initConnBackoffManager.Backoff().C()
					continue
				default:
					logger.Warnf("%s: watch of %v ended with: %v", r.name, r.expectedTypeName, err)
				}
			}
			return nil
		}
...
loop:
	...
	logger.Infof("%s: Watch close - %v total %v items received", r.name, r.expectedTypeName, eventCount)
	return nil
}
