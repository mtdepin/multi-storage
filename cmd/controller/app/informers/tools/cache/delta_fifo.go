...
func (f *DeltaFIFO) queueActionLocked(actionType DeltaType, obj interface{}) error {
	...
	if len(newDeltas) > 0 {
		...
		if oldDeltas == nil {
			logger.Errorf("Impossible dedupDeltas for id=%q: oldDeltas=%#+v, obj=%#+v; ignoring", id, oldDeltas, obj)
			return nil
		}
		logger.Errorf("Impossible dedupDeltas for id=%q: oldDeltas=%#+v, obj=%#+v; breaking invariant by storing empty Deltas", id, oldDeltas, obj)
		f.items[id] = newDeltas
...
func (f *DeltaFIFO) Pop(process PopProcessFunc) (interface{}, error) {
	...
	for {
		...
		if !ok {
			...
			logger.Errorf("Inconceivable! %q was in f.queue but not f.items; ignoring.", id)
			continue
		}
		delete(f.items, id)
...
func (f *DeltaFIFO) Replace(list []interface{}, _ string) error {
	...
	for _, k := range knownKeys {
		...
		if err != nil {
			deletedObj = nil
			logger.Errorf("Unexpected error %v during lookup of key %v, placing DeleteFinalStateUnknown marker without object", err, k)
		} else if !exists {
			deletedObj = nil
			logger.Infof("Key %v does not exist in known objects store, placing DeleteFinalStateUnknown marker without object", k)
		}
		queuedDeletions++
		if err := f.queueActionLocked(Deleted, DeletedFinalStateUnknown{k, deletedObj}); err != nil {
			return err
...
func (f *DeltaFIFO) syncKeyLocked(key string) error {
	...
	if err != nil {
		logger.Errorf("Unexpected error %v during lookup of key %v, unable to queue object for sync", err, key)
		return nil
	} else if !exists {
		logger.Infof("Key %v does not exist in known objects store, unable to queue object for sync", key)
		return nil
	}
