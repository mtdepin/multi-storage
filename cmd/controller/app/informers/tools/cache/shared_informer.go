...
// is not engineered to deal well with a large baclogger of
...
func WaitForNamedCacheSync(controllerName string, stopCh <-chan struct{}, cacheSyncs ...InformerSynced) bool {
	logger.Infof("Waiting for caches to sync for %s", controllerName)
	...
	logger.Infof("Caches are synced for %s ", controllerName)
	return true
}
...
func WaitForCacheSync(stopCh <-chan struct{}, cacheSyncs ...InformerSynced) bool {
	err := wait.PollImmediateUntil(syncedPollPeriod,
		...
		stopCh)
	if err != nil {
		logger.Infof("stop requested")
		return false
	}

	logger.Infof("caches populated")
	return true
}
...
func (s *sharedIndexInformer) Run(stopCh <-chan struct{}) {
	...
	if s.HasStarted() {
		logger.Warnf("The sharedIndexInformer has started, run more than once is not allowed")
		return
	}
...
func determineResyncPeriod(desired, check time.Duration) time.Duration {
	...
	if check == 0 {
		logger.Warnf("The specified resyncPeriod %v is invalid because this shared informer doesn't support resyncing", desired)
		return 0
	}
	if desired < check {
		logger.Warnf("The specified resyncPeriod %v is being increased to the minimum resyncCheckPeriod %v", desired, check)
		return check
	}
	return desired
}
...
func (s *sharedIndexInformer) AddEventHandlerWithResyncPeriod(handler ResourceEventHandler, resyncPeriod time.Duration) {
	...
	if s.stopped {
		logger.Infof("Handler %v was not added to shared informer because it has stopped already", handler)
		return
	}

	if resyncPeriod > 0 {
		if resyncPeriod < minimumResyncPeriod {
			logger.Warnf("resyncPeriod %v is too small. Changing it to the minimum allowed value of %v", resyncPeriod, minimumResyncPeriod)
			resyncPeriod = minimumResyncPeriod
		}

		if resyncPeriod < s.resyncCheckPeriod {
			if s.started {
				logger.Warnf("resyncPeriod %v is smaller than resyncCheckPeriod %v and the informer has already started. Changing it to %v", resyncPeriod, s.resyncCheckPeriod, s.resyncCheckPeriod)
