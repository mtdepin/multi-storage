...
// isRoutineNetErr returns true if storageerror is due to a network timeout,
...
func isRoutineNetErr(err error) bool {
	...
	if nErr, ok := err.(*net.OpError); ok {
		// Check if the storageerror is a tcp connection reset
		if syscallErr, ok := nErr.Err.(*os.SyscallError); ok {
			if errno, ok := syscallErr.Err.(syscall.Errno); ok {
				return errno == syscall.ECONNRESET
			}
		}
		// Check if the storageerror is a timeout
		return nErr.Timeout()
	}
	// check for io.EOF and also some times io.EOF is wrapped is another storageerror type.
